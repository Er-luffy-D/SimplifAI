...
import { FileText, Upload, CheckCircle2, Sparkles, Image as ImageIcon } from "lucide-react";
...
export function FileUploader() {
    ...
    const isImageFile = (file: File) => file.type.startsWith("image/") || file.name.endsWith(".svg");
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const progressInterval = setInterval(() => {
            setProgress((prev) => (prev >= 60 ? prev : prev + Math.random() * 10 + Math.random() * 5));
        }, 200);
        ...
        try {
            ...
            setTimeout(() => router.push(`/results/${res.data.id}`), 800);
        } catch (e) {
                        ...
                        {["short", "medium", "long"].map((len) => (
                            <Button
                                ...
                                onClick={() => {
                                    ...
                                    if (len !== "short") handleInfo("Processing may take longer for larger summaries.");
                                }}
                                className={`flex-1 min-w-[120px] py-1 ${summaryLength === len ? "bg-violet-500 hover:bg-violet-700" : ""}`}
                                data-cursor="hover"
                                data-cursor-text={`${len.charAt(0).toUpperCase() + len.slice(1)} Summary`}
                            >
                                {len.charAt(0).toUpperCase() + len.slice(1)}{" "}
                                <span className="text-xs ml-1 opacity-75">
                                    {len === "short" ? "(3-4 points)" : len === "medium" ? "(6-8 points)" : "(9-10 points)"}
                                </span>
                            </Button>
                        ))}
                    </div>
                ...
                >
                    {file ? (
                                ...
                                {previewUrl ? (
                                    <div className="relative w-24 h-24">
                                        <Image
                                            ...
                                            alt="Preview"
                                            fill
                                            className="object-cover rounded-lg border-2 border-green-500 shadow-lg"
                                        />
                                        <CheckCircle2 className="w-5 h-5 text-green-500 absolute -top-1 -right-1 animate-scale-in bg-white rounded-full" />
                                    </div>
                                ) : (
                                    <>
                        ...
                        <div className="flex flex-col items-center gap-3 text-center px-2">
                            <div className="relative">
                                <Upload className={`w-12 h-12 text-muted-foreground transition-all duration-300 ${dragActive ? "scale-110 text-primary" : ""}`} />
                            ...
                            <div className="space-y-1">
                                ...
                                <div className="flex items-center gap-2 justify-center text-xs text-muted-foreground mt-2">
                                    <span className="flex items-center gap-1">
                                        <FileText className="w-3 h-3" /> PDF, DOCX, TXT, HTML
                                    </span>
                                    <span>•</span>
                                    <span className="flex items-center gap-1">
                                        <ImageIcon className="w-3 h-3" /> JPG, PNG, WEBP, SVG
                                    </span>
                                </div>
                            </div>
                        </div>
                    )}
                    <Input id="file" type="file" className="hidden" accept=".pdf,.txt,.docx,.html,.svg,image/*" onChange={handleFileChange} />
